...
def initprop():
    ...
    bpy.types.Scene.gx_custom_object_b = bpy.props.BoolProperty(
        name = "Use Custom Object",
        default=False,
        description = "use custom object or not")

    bpy.types.Scene.gx_custom_object = bpy.props.StringProperty(
        name = "Custom Object",
        default="",
        description = "Object as array part",
    ...
    bpy.types.Scene.gx_array_merge = bpy.props.BoolProperty(
        name = "Merge",
        default = True,
        description = "Merge the objects of an array",
        update=update_merge_array)
...
def initpropvalues():
    ...
    bpy.context.scene['gx_custom_object_b']=False
    bpy.context.scene['gx_start'] = 100
    bpy.context.scene['gx_space_array'] = 1.7
    bpy.context.scene['gx_array_merge'] = True
...
def update_custom_object(self, context):
   pass
...
def update_merge_array(self, context):
    for i in range(bpy.context.scene['gx_count_x']):
        if bpy.context.scene['gx_channels'] == 1 or bpy.context.scene['gx_channels'] == 0:
            name = "bar_l_" + str(i+1)
            bpy.data.objects[name].modifiers['Array'].use_merge_vertices = bpy.context.scene['gx_array_merge']
        if bpy.context.scene['gx_channels'] == 2 or bpy.context.scene['gx_channels'] == 0:
            name = "bar_r_" + str(i+1)
            bpy.data.objects[name].modifiers['Array'].use_merge_vertices = bpy.context.scene['gx_array_merge']
...
def generate_objects(i):
    ...
    if bpy.context.scene['gx_channels'] == 1 or bpy.context.scene['gx_channels'] == 0:
        ...
        if bpy.context.scene['gx_custom_object_b']==True:
            try:
                obname = bpy.context.scene['gx_custom_object']
                old_ob = bpy.data.objects[obname]
                new_ob = old_ob.copy()
